...
def choose_ride(df, previous_rides, **kwargs):
    ...
    df = df[
        df.apply(
            lambda row: all(
                (row[k] == v if not callable(v) else v(row[k]))
...
def choose_pz_endurance(previous_rides, length=None):
    return choose_ride(
        df,
        previous_rides,
        **{"Type No Theme": "Power Zone Endurance", "Length": length},
    )
...
def choose_pz_no_z5z7(previous_rides):
    return choose_ride(
        ...
        previous_rides,
        **{
            "Type No Theme": "Power Zone",
            "Z5 Length": 0,
            "Z6 Length": 0,
            "Z7 Length": 0,
            "Length": lambda x: 45 <= x <= 60,
        },
    )
...
def choose_pz_with_z5z7(previous_rides):
    df_pz = df[
        (df["Type No Theme"] == "Power Zone")
        & ((df["Z5 Length"] > 0) | (df["Z6 Length"] > 0) | (df["Z7 Length"] > 0))
        & (df["Length"].between(45, 60))
    ]
    if df_pz.empty:
        raise Exception("No rides found matching criteria.")
